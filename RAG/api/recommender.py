...
from RAG.api.utils import haversine, format_time_info
from RAG.api.config import DB_CONFIG, INFRA_TYPES
...
class InfraDataAccessor:
    ...
    def get_infra_data(self, infra_type):
        ...
        try:
            ...
            if not table_exists:
                print(
                    f"{infra_type} 테이블이 존재하지 않습니다. 가상 데이터를 사용합니다."
                )
                cursor.close()
...
def filter_properties_by_location(properties, user_state):
    ...
    if service == "반경":
        ...
        # 가장 가까운 역 3개 찾기
        dist_list = [
            (st, haversine(lat0, lng0, c["lat"], c["lng"]))
            for st, c in station_coords.items()
        ]
...
def filter_properties_by_budget(properties, user_state):
    ...
    print(
        f"예산 필터링 시작: 월세 {rent_limit}만원, 보증금 {deposit_limit}만원, 관리비 {maint_limit}만원"
    )
...
def filter_properties_by_extended_budget(properties, user_state, multiplier=1.3):
    ...
    print(
        f"확장 예산 필터링: 월세 {rent_limit:.0f}만원, 보증금 {deposit_limit:.0f}만원, 관리비 {maint_limit:.0f}만원"
    )
...
def filter_properties_by_features(properties, user_state):
        ...
        if passes_filters:
            ...
            try:
                ...
                if "5평 이하" in size_pref and size > 5:
                    ...
                    filter_reason = (
                        f"{md.get('address')} - 10~15평 범위가 아니라서 제외"
                    )
                    passes_filters = False
                elif "15~20평" in size_pref and (size < 15 or size > 20):
                    filter_reason = (
                        f"{md.get('address')} - 15~20평 범위가 아니라서 제외"
                    )
                    passes_filters = False
    ...
    if len(filtered) == 0 and filtering_results:
        ...
        # 가장 많은 필터링 원인 식별
        filter_counts = {"층수": floor_count, "방향": direction_count}
...
def apply_infra_scores(properties, infra_preferences, infra_data):
    ...
    print(
        f"인프라 점수 계산 시작: {len(properties)}개 매물, {len(sorted_infra)}개 인프라 유형"
    )
    ...
    for prop in properties:
        ...
        prop_info = {
            ...
            "view": md.get(
                "조망", md.get("direction", md.get("view", "조망 정보 없음"))
            ),
        ...
        for infra_type, weight in sorted_infra:
            ...
            if station_name and infra_type == "traffic_subway":
                ...
                matched_stations = [
                    ...
                    if station_name in item["name"] or item["name"] in station_name
                ]
        ...
        prop_info["infra_score"] = (
            min(5.0, max(0.5, total_score)) if total_score > 0 else 0
        )
...
def get_location_based_properties_with_extended_budget(
    properties, user_state, lat0, lng0, multiplier=1.3
):
    ...
    if service == "반경":
        radius = user_state.get("radius", 1000)
        print(
            f"위치+확장예산 필터링: 반경 {radius}m, 예산 {rent_limit:.0f}/{deposit_limit:.0f}/{maint_limit:.0f}만원"
        )
        ...
        key = "walk_time" if movement == "도보" else "transit_time"
        print(
            f"위치+확장예산 필터링: {movement} {time_limit}분, 예산 {rent_limit:.0f}/{deposit_limit:.0f}/{maint_limit:.0f}만원"
        )
...
class RealEstateRecommender:
    ...
    def search_properties(self):
        ...
        try:
            ...
            resp = self.index.query(
                vector=[0.0] * 1536, top_k=500, include_metadata=True
            )
    ...
    def get_recommendations(self, is_retry=False):
        ...
        try:
            ...
            # 1. 위치 기반 필터링
            location_filtered = filter_properties_by_location(
                all_properties, self.user_state
            )
            print(f"위치 조건 충족 매물 수: {len(location_filtered)}")

            # 2. 예산 기반 필터링
            budget_filtered = filter_properties_by_budget(
                all_properties, self.user_state
            )
            print(f"예산 조건 충족 매물 수: {len(budget_filtered)}")

            # 3. 매물 특성 필터링 (주차, 엘리베이터 제외 - 소프트 스코어링으로 변경)
            feature_filtered, feature_filtering_results = filter_properties_by_features(
                all_properties, self.user_state
            )
            ...
            for prop in all_properties:
                ...
                if (
                    prop_id in location_ids
                    and prop_id in budget_ids
                    and prop_id in feature_ids
                ):
            ...
            if len(combined_scored) < 5:
                print(
                    f"종합 추천 매물이 {len(combined_scored)}개로 5개 미만입니다. 조건을 완화합니다."
                )

                # 조건 완화 원인 분석 및 안내
                self.provide_recommendation_guidance(
                    location_filtered, budget_filtered, feature_filtered
                )
                ...
                if len(combined_scored) < 5 and location_scored:
                    ...
                    if added_from_location > 0:
                        print(
                            f"위치 기반 매물에서 {added_from_location}개 추가했습니다."
                        )
                ...
                if len(combined_scored) < 5:
                    ...
                    extended_budget_props = (
                        get_location_based_properties_with_extended_budget(
                            all_properties,
                            self.user_state,
                            lat0,
                            lng0,
                    ...
                    # 특성 필터링 적용 (옵션)
                    extended_budget_props_filtered, _ = filter_properties_by_features(
                        extended_budget_props, self.user_state
                    )
                    ...
                    for prop in extended_scored:
                        if (
                            prop.get("id") not in existing_ids
                            and added_from_extended < need_more
                        ):
                            combined_scored.append(prop)
                            existing_ids.add(prop.get("id"))
                    ...
                    if added_from_extended > 0:
                        print(
                            f"위치+확장예산 매물에서 {added_from_extended}개 추가했습니다."
                        )
                ...
                if len(combined_scored) < 5 and feature_scored:
                    ...
                    if lat0 and lng0:
                        ...
                        for prop in additional_from_feature:
                            ...
                            if plat and plng:
                                try:
                                    prop["_distance"] = haversine(
                                        lat0, lng0, float(plat), float(plng)
                                    )
                                except:
                    ...
                    if added_from_feature > 0:
                        print(
                            f"매물 특성 기반 매물에서 {added_from_feature}개 추가했습니다."
                        )
                ...
                if len(combined_scored) < 5:
                    ...
                    if added_from_infra > 0:
                        print(
                            f"인프라 점수 기반 매물에서 {added_from_infra}개 추가했습니다."
                        )

                # 최종 추가된 매물 수 출력
                total_added = len(combined_scored) - original_count
                print(
                    f"총 {total_added}개 매물이 조건 완화를 통해 종합 추천에 추가되었습니다."
                )
    ...
    def provide_recommendation_guidance(
        self, location_filtered, budget_filtered, feature_filtered
    ):
        ...
        if len(feature_filtered) < 5:
            ...
            # 가장 제한적인 특성 조건 파악
            floor_filtered = any(
                "층" in r and "층수" not in r for r in self.filtering_results
            )
        ...
        # 종합 분석
        if (
            len(location_filtered) >= 5
            and len(budget_filtered) >= 5
            and len(feature_filtered) >= 5
        ):
            print(
                "  • 모든 조건은 개별적으로 충족하는 매물이 충분하지만, 모든 조건을 동시에 만족하는 매물이 부족합니다."
            )
            print(
                "    💡 추천: 가장 중요한 조건은 유지하고 덜 중요한 조건을 완화해보세요."
            )
