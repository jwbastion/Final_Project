...
from RAG.api.api import api_bp
import argparse
import os
from RAG.api.chatbot import RealEstateChatbot
from RAG.api.config import (
    INFRA_TYPES,
...
def run_cli():
    ...
    while True:
                ...
                if selected_infra_types:
                    ...
                    if INFRA_DETAIL_QUESTIONS.get(current_infra_type):
                        infra_name = next(
                            (
                                x["name"]
                                for x in INFRA_TYPES
                                if x["code"] == current_infra_type
                            ),
                            current_infra_type,
                        )
                        print(f"\n🤖 챗봇: {infra_name}에 대한 추가 질문입니다.")
                        print(
                            INFRA_DETAIL_QUESTIONS[current_infra_type][
        ...
        # 인프라 세부 질문 처리
            ...
            infra_name = next(
                (x["name"] for x in INFRA_TYPES if x["code"] == current_infra_type),
                current_infra_type,
            )
            question = INFRA_DETAIL_QUESTIONS[current_infra_type][
                current_question_index
            ]
            chatbot.user_state.update(
                f"infra_detail_{current_infra_type}_{current_question_index}",
                user_input,
            )
        ...
        # 매물 특성 질문 처리
            ...
            if current_feature_index < len(PROPERTY_FEATURE_QUESTIONS):
                ...
                if current_feature_index < len(PROPERTY_FEATURE_QUESTIONS):
                    ...
                    # 인프라 선호도 저장
                    chatbot.user_state.update(
                        "infra_preferences", user_infra_preferences
                    )

                    # 매물 정보 출력 함수 정의
                    def print_property(
                        index, prop, detailed=True, user_preferences=None
                    ):
                        ...
                        print(
                            f"     💸 월세: {prop.get('rent', 0)}만원 | 보증금: {prop.get('deposit', 0)}만원 | 관리비: {prop.get('maint', 0)}만원"
                        )
                        print(
                            f"     🚶 {prop.get('time_info', '시간 정보 없음')} | 인프라 점수: {'⭐' * stars}"
                        )
                        ...
                        if detailed:
                            ...
                            # 사용자 선호도에 맞는지 확인
                            if user_preferences:
                                feature_prefs = user_preferences.get(
                                    "property_features", {}
                                )
                                ...
                                # 난방 방식 일치 여부
                                heating = prop.get(
                                    "heating_type", prop.get("난방", "정보 없음")
                                )
                                ...
                                if isinstance(parking_text, str):
                                    parking = (
                                        "있" in parking_text.lower()
                                        or "가능" in parking_text.lower()
                                    )
                                else:
                                ...
                                if parking_pref and parking_pref != "상관없음":
                                    parking_required = (
                                        "있" in parking_pref
                                        or "중요" in parking_pref
                                        or "네" in parking_pref
                                        or "필요" in parking_pref
                                    )
                                    if parking_required:
                                ...
                                elevator = (
                                    "있" in str(elevator_text).lower()
                                    or "가능" in str(elevator_text).lower()
                                )

                                elevator_pref = feature_prefs.get(
                                    "elevator", ""
                                ).lower()
                                elevator_match = True
                                ...
                                if elevator_pref and elevator_pref != "상관없음":
                                    elevator_required = (
                                        "있" in elevator_pref
                                        or "중요" in elevator_pref
                                        or "네" in elevator_pref
                                        or "필요" in elevator_pref
                                    )
                                ...
                                # 방향 일치 여부
                                direction = prop.get(
                                    "view", prop.get("direction", "정보 없음")
                                ).lower()
                                direction_pref = feature_prefs.get(
                                    "direction", ""
                                ).lower()
                                ...
                                # 매물 정보 출력
                                print(
                                    f"     🏢 층수: {floor} ({floor_category}) {floor_icon} | 면적: {size}평 {size_icon}"
                                )
                                print(
                                    f"     🔥 난방: {heating} {heating_icon} | 방향: {prop.get('view', prop.get('direction', '정보 없음'))} {direction_icon}"
                                )
                                print(
                                    f"     🅿️ 주차: {parking_display} {parking_icon} | 엘리베이터: {elevator_display} {elevator_icon}"
                                )
                            else:
                                # 기본 출력 방식 (선호도 체크 없음)
                                print(
                                    f"     🏢 층수: {floor} ({floor_category}) | 면적: {size}평"
                                )
                                print(
                                    f"     🔥 난방: {prop.get('heating_type', prop.get('난방', '정보 없음'))} | 방향: {prop.get('view', prop.get('direction', '정보 없음'))}"
                                )
                                parking_display = (
                                    "가능" if prop.get("parking", False) else "불가능"
                                )
                                elevator_text = prop.get("엘리베이터", "정보 없음")
                                elevator_display = (
                                    "있음"
                                    if "있" in str(elevator_text).lower()
                                    else "없음"
                                )
                                print(
                                    f"     🅿️ 주차: {parking_display} | 엘리베이터: {elevator_display}"
                                )

                            # 시설 정보
                            if prop.get("facilities", prop.get("생활시설", "")):
                                facilities = prop.get(
                                    "facilities", prop.get("생활시설", "")
                                )
                            ...
                            if prop.get("infra_details"):
                                ...
                                # 각 인프라 유형별 정보
                                for infra_type, detail in prop.get(
                                    "infra_details", {}
                                ).items():
                                        ...
                                        try:
                                            ...
                                            infra_name = next(
                                                (
                                                    x["name"]
                                                    for x in INFRA_TYPES
                                                    if x["code"] == infra_type
                                                ),
                                                infra_type,
                                            )
                                        except:
                                            infra_name = infra_type.replace(
                                                "_", " "
                                            ).title()
                                ...
                                if near_infras:
                                    ...
                                    for infra_name, nearest, distance in near_infras:
                                        print(
                                            f"       - {infra_name}: {nearest} ({distance:.0f}m)"
                                        )
                                else:
                    ...
                    try:
                        ...
                        try:
                            # 속성이 존재하는지 확인
                            if hasattr(
                                chatbot.user_state, "save_recommendations_to_db"
                            ):
                                db_save_result = (
                                    chatbot.user_state.save_recommendations_to_db(
                                        recommendations
                                ...
                                if db_save_result:
                                    print(
                                        "📊 추천 매물이 DB에 성공적으로 저장되었습니다."
                                    )
                            else:
                                print(
                                    "⚠️ DB 저장 기능을 사용할 수 없습니다. 결과는 화면에만 표시됩니다."
                                )
                        ...
                        if chatbot.user_state.get("service") == "반경":
                            ...
                            print(
                                f"  소요시간: {chatbot.user_state.get('time_limit')}분 ({chatbot.user_state.get('movement')})"
                            )
                        print(
                            f"  주소: {chatbot.user_state.get('address', '설정된 주소 없음')}"
                        )
                        print(
                            f"  선호 지역: {chatbot.user_state.get('preferred_area', '설정된 선호 지역 없음')}"
                        )

                        # 추천 매물이 있는지 확인
                        has_recommendations = (
                            recommendations.get("combined")
                            or recommendations.get("location_based")
                            or recommendations.get("budget_based")
                        )
                        ...
                        if has_recommendations:
                            ...
                            if recommendations.get("combined", []):
                                ...
                                sorted_properties = sorted(
                                    ...
                                    key=lambda x: x.get(
                                        "total_score", x.get("infra_score", 0)
                                    ),
                                    reverse=True,
                                )
                                ...
                                for prop in sorted_properties:
                                    if "total_score" not in prop:
                                        ...
                                        prop["total_score"] = (
                                            prop.get("infra_score", 0) + feature_score
                                        )
                                ...
                                if sorted_properties:
                                    for i, prop in enumerate(sorted_properties[:5], 1):
                                        ...
                                        print(
                                            f"  # 매물 {i} - 총점: {prop.get('total_score', prop.get('infra_score', 0)):.1f}{feature_info}"
                                        )
                                        ...
                                        print_property(
                                            ...
                                            prop,
                                            detailed=True,
                                            user_preferences=chatbot.user_state.state,
                                        )
                                else:
                                    print(
                                        "⚠️ 조건에 맞는 매물이 없습니다. 설정을 변경해보세요."
                                    )
                            ...
                            if recommendations.get("location_based", []):
                                ...
                                for i, prop in enumerate(
                                    recommendations["location_based"][:3], 1
                                ):
                                    print_property(
                                        i,
                                        prop,
                            ...
                            if recommendations.get("budget_based", []):
                                print("\n🏠 [예산 기반 추천 매물]")
                                for i, prop in enumerate(
                                    recommendations["budget_based"][:3], 1
                                ):
                                    print_property(
                                        i,
                                        prop,
                            ...
                            print("\n💡 추천 사항:")
                            print(
                                "  1. 예산 범위를 넓혀보세요 (월세, 보증금 상향 조정)"
                            )
                            print(
                                "  2. 검색 반경을 넓혀보세요 (현재 반경 → 더 넓은 범위)"
                            )
                        ...
                        if has_recommendations:
                            ...
                            if favorites_input.strip():
                                ...
                                if "," in favorites_input:
                                    favorite_numbers = [
                                        int(s.strip())
                                        for s in favorites_input.split(",")
                                        if s.strip().isdigit()
                                    ]
                                else:
                                    favorite_numbers = [
                                ...
                                if valid_numbers:
                                    ...
                                    user_id = chatbot.user_state.get(
                                        "user_uuid", "default_user"
                                    )
                                    filename = os.path.join(
                                        favorites_dir, f"{user_id}_favorites.json"
                                    )
                        ...
                        if view_favorites in ["y", "yes", "네", "예"]:
                            ...
                            filename = os.path.join(
                                "favorites", f"{user_id}_favorites.json"
                            )
