...
def print_property(index, prop, detailed=True, user_preferences=None):
    ...
    print(
        f"     ⭐ 총점: {total_score:.1f}/10.0 = 인프라({infra_score:.1f}/3.0) + 특성({feature_score:.1f}/7.0)"
    )
    ...
    if detailed:
        ...
        if any(
            term in floor_str.lower() for term in ["반지하", "반지층", "반층", "지하"]
        ):
        ...
        if user_preferences:
            ...
            preferences_match = check_preferences_match(
                prop, feature_prefs, floor_category
            )
            ...
            # 매물 세부 정보 출력
            print(
                f"     🏢 층수: {floor_str} ({floor_category}) {preferences_match['floor_icon']} | 면적: {size}평 {preferences_match['size_icon']}"
            )
            print(
                f"     🔥 난방: {prop.get('heating_type', prop.get('난방', '정보 없음'))} {preferences_match['heating_icon']} | "
                f"방향: {prop.get('view', prop.get('direction', '정보 없음'))} {preferences_match['direction_icon']}"
            )
            print(
                f"     🅿️ 주차: {preferences_match['parking_display']} {preferences_match['parking_icon']} | "
...
def check_preferences_match(prop, feature_prefs, floor_category=None):
    ...
    if floor_pref and floor_pref != "상관없음":
        if "2층 이상" in floor_pref:
            ...
            floor_match = (
                "반지하" not in floor_category and "반층" not in floor_category
            )
    ...
    parking = (
        "있" in str(parking_text).lower()
        or "가능" in str(parking_text).lower()
        or parking_text is True
    )
    ...
    elevator_text = prop.get("엘리베이터", "정보 없음")
    elevator = (
        "있" in str(elevator_text).lower() or "가능" in str(elevator_text).lower()
    )
        ...
        for infra_type, detail in prop.get("infra_details", {}).items():
            if detail and detail.get("nearest"):
                ...
                try:
                    ...
                    infra_name = next(
                        (x["name"] for x in INFRA_TYPES if x["code"] == infra_type),
                        infra_type,
                    )
                except:
...
def run_cli():
    ...
    user_uuid = (
        args.uuid
        or input("사용자 UUID를 입력하세요 (빈 값 입력시 기본값 사용): ").strip()
        or None
    )
            ...
            if INFRA_DETAIL_QUESTIONS.get(infra_type):
                ...
                for q_idx, question in enumerate(INFRA_DETAIL_QUESTIONS[infra_type]):
                    ...
                    chatbot.user_state.update(
                        f"infra_detail_{infra_type}_{q_idx}", answer
                    )
    ...
    try:
        ...
        try:
            if hasattr(chatbot.user_state, "save_recommendations_to_db"):
                db_save_result = chatbot.user_state.save_recommendations_to_db(
                    recommendations
                )
                if db_save_result:
        ...
        if has_recommendations:
            ...
            if recommendations.get("combined"):
                ...
                within_budget = [
                    p
                    for p in combined_properties
                    if not p.get("budget_exceeded", False)
                ]
                exceeds_budget = [
                    p for p in combined_properties if p.get("budget_exceeded", False)
                ]
...
def handle_favorites(chatbot, properties):
    ...
    if "," in favorites_input:
        favorite_numbers = [
            int(s.strip()) for s in favorites_input.split(",") if s.strip().isdigit()
        ]
    else:
        favorite_numbers = [
            int(s.strip()) for s in favorites_input.split() if s.strip().isdigit()
    ...
    # 유효한 번호만 필터링
    valid_numbers = [
        n for n in favorite_numbers if 1 <= n <= len(all_displayed_properties)
    ]
    ...
    for fav in saved_favorites:
        ...
        if not any(
            (
                ...
                or existing.get("address") == fav.get("address")
            )
