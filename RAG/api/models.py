...
class UserState:
            ...
            if self._load_from_file(user_uuid):
                ...
                try:
                    ...
                    if user_data:
                        ...
                        if "preferred_area" in user_data and user_data["preferred_area"]:
                            essential_fields["preferred_area"] = user_data["preferred_area"]
    ...
    def add_to_history(self, user_message, bot_response):
        ...
        if self.state.get("user_uuid"):
            try:
                ...
                if result:
                    print(f"대화 기록이 DB에 성공적으로 저장되었습니다. (UUID: {self.state.get('user_uuid')[:8]}...)")
                else:
    ...
    def save_chat_history(self, user_message, bot_response):
        ...
        try:
            print(f"DB 연결 시도: {DB_CONFIG.get('host')}:{DB_CONFIG.get('port')}/{DB_CONFIG.get('database')}")
            conn = psycopg2.connect(**DB_CONFIG)
            cursor = conn.cursor()
    ...
    def save_recommendations_to_db(self, recommendations):
        ...
        try:
            ...
            # 각 테이블에 데이터 저장
            combined_count = save_properties("combined_recommendations", recommendations.get("combined", []))
            location_count = save_properties("location_recommendations", recommendations.get("location_based", []))
            budget_count = save_properties("budget_recommendations", recommendations.get("budget_based", []))
            
            conn.commit()
            cursor.close()
            conn.close()
            
            print(f"DB에 저장 완료: 종합 {combined_count}개, 위치기반 {location_count}개, 예산기반 {budget_count}개")
            return True
