...
class RealEstateRecommender:
    def __init__(self, index, user_state, data_accessor):
        self.index = index
    ...
    def get_recommendations(self, is_retry=False):
        ...
        # 검색된 매물이 없을 경우 조건 완화 시도
        if not all_properties:
            print("검색된 매물이 없습니다. 조건을 완화하여 재시도합니다...")
            # 조건 완화 로직 실행
            self._relax_conditions()
            # 재검색
    ...
    def _relax_conditions(self):
        """검색 조건 완화 메서드"""
        # 1. 반경 확장 (원래 반경의 1.5배)
        if self.user_state.get("service") == "반경":
            original_radius = self.user_state.get("radius", 500)
            new_radius = int(original_radius * 1.5)
            self.user_state.update("radius", new_radius)
            print(f"검색 반경을 {original_radius}m에서 {new_radius}m로 확장합니다.")
        
        # 2. 소요시간 확장 (5분 추가)
        elif self.user_state.get("service") == "소요시간":
            original_time = self.user_state.get("time_limit", 30)
            new_time = original_time + 5
            self.user_state.update("time_limit", new_time)
            print(f"소요시간을 {original_time}분에서 {new_time}분으로 확장합니다.")
        ...
        # 4. 보증금 범위 확장 (10% 증가)
        original_deposit = self.user_state.get("deposit", 1000)
        new_deposit = int(original_deposit * 1.1)
        self.user_state.update("deposit", new_deposit)
        print(f"보증금 범위를 {original_deposit}만원에서 {new_deposit}만원으로 확장합니다.")
